...
class User(UserMixin, db.Model):
    ...
    def get_pending_count(self):
        return FriendRequest.query.filter_by(to_user_id=self.id, status='pending').count()
    ...
    def get_unread_from(self, user_id):
        return Message.query.filter_by(sender_id=user_id, receiver_id=self.id, is_read=False).count()

    def has_sent_request_to(self, user):
        return FriendRequest.query.filter_by(from_user_id=self.id, to_user_id=user.id, status='pending').count() > 0

    def has_request_from(self, user):
        return FriendRequest.query.filter_by(from_user_id=user.id, to_user_id=self.id, status='pending').count() > 0
...
class UserProgress(db.Model):
    ...
    def completion_percent(self, total_lessons):
        ...
        return int((completed + passed) / (total_lessons * 2) * 100)
